<div>
    <div
        ...
        <div class="search-global" style="width:300px">
            ...
            <input type="text" wire:model.live.debounce.300ms="buscar" placeholder="Buscar zona...">
        </div>
        <button wire:click="openCreateModal" class="btn btn-primary">
            <i data-lucide="plus"></i> Nueva Zona
        </button>
    </div>
        ...
        <table>
            ...
            <tbody>
                @forelse ($zonas as $zona)
                    <tr>
                        <td style="color:var(--text-muted)">{{ $zona->id }}</td>
                        <td><strong>{{ $zona->nombre }}</strong></td>
                        <td style="color:var(--text-muted)">{{ $zona->descripcion ?? '—' }}</td>
                        <td style="text-align:center">
                            <button wire:click="toggleActivo({{ $zona->id }})"
                                class="badge {{ $zona->activo ? 'badge-success' : 'badge-danger' }}"
                        ...
                        <td style="text-align:center">
                            <div style="display:flex;gap:4px;justify-content:center">
                                <button wire:click="openEditModal({{ $zona->id }})" class="btn btn-sm btn-secondary"><i
                                        data-lucide="pencil"></i></button>
                                <button wire:click="delete({{ $zona->id }})" wire:confirm="¿Eliminar esta zona?"
                ...
                @empty
                    <tr>
                        <td colspan="5" style="text-align:center;padding:60px;color:var(--text-muted)">
                            <i data-lucide="map-pin" style="width:48px;height:48px;display:block;margin:0 auto 12px"></i>
    ...
    <div style="margin-top:16px">{{ $zonas->links() }}</div>

    @if($showModal)
        <div class="modal-backdrop" wire:click.self="$set('showModal', false)">
            <div class="modal" style="max-width:480px">
                <div class="modal-header">
                    <h3 style="font-weight:700">{{ $editMode ? 'Editar Zona' : 'Nueva Zona' }}</h3>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" wire:model="nombre" class="form-input" placeholder="Ej: Zona Norte, Lima Este">
